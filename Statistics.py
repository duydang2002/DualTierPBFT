...
class Statistics:
    ...
    def blocks_results():
        ...
        Statistics.totalBlocks= len(Statistics.chain)-1
        #Statistics.staleBlocks = Statistics.totalBlocks - Statistics.mainBlocks
        for b in Statistics.chain:
            if type(b.transactions) != float:
                trans += len(b.transactions)
            else : trans += b.transactions
        #if Statistics.staleBlocks > 0 and Statistics.totalBlocks >0:
        #    Statistics.staleRate= round(Statistics.staleBlocks/Statistics.totalBlocks * 100, 2)
        #else: Statistics.staleRate=0
        #if p.model==2: Statistics.uncleRate= round(Statistics.uncleBlocks/Statistics.totalBlocks * 100, 2)
        #else: Statistics.uncleRate==0
        Statistics.blockData = [ Statistics.totalBlocks, Statistics.mainBlocks,  Statistics.staleBlocks, Statistics.staleRate, trans, trans/p.simTime]
    ...
    def global_chain():
        if p.model==0 or p.model==1:
                ...
                for i in Statistics.chain[1:]:
                    block= [i.depth, i.id, i.previous, i.miner, i.transactions if isinstance(i.transactions, float) else len(i.transactions), i.is_empty, i.seed, i.block_hash]
                    Statistics.chain_modified +=[block]
    ...
    def print_to_excel(fname):
        ...
        df2.columns= ['Total Blocks', 'Main Blocks', 'Stale Blocks', 'Stale Rate', '# transactions', 'TPS']
        ...
        df4 = pd.DataFrame(Statistics.chain_modified)
        ...
        else: df4.columns= ['Block Depth', 'Block ID', 'Previous Block', 'Miner ID', '# transactions', 'Empty', 'Seed', 'Block hash']
    ...
    def reset():
        ...
        Statistics.index=0
        Statistics.chain=[]
        Statistics.chain_modified =[]
        Statistics.total_transaction= 0
